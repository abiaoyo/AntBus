...
final public class AntBusListener {
    ...
    public func listening(keyPath:String, for object:AnyObject, handler:AntBusListenerHandler!) {
        var container:AntBusListenerContainer? = objc_getAssociatedObject(object, &AntBusListener_container_key) as? AntBusListenerContainer
        if container == nil {
            container = AntBusListenerContainer.init(target: object)
            objc_setAssociatedObject(object, &AntBusListener_container_key, container, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
            AntBusDeallocHook.shared.installDeallocHook(for: object, propertyKey: "AntBusListener", handlerKey: "AntBusListenerHandler") { hkeys in
                container?.removeKVO()
            }
        }
...
private class AntBusListenerContainer: NSObject {
    ...
    var container = NSMutableDictionary.init()
    ...
    func addMonitoring(keyPath:String, handler:AntBusListenerHandler!) {
        ...
        if handlerContainers == nil {
            handlerContainers = NSMutableArray.init()
