...
//MARK:IAntBusChannel
public protocol IAntBusChannel {
    var data:IAntBusData { get }
    var router:IAntBusRouter { get }
    var service:IAntBusService { get }
    var notification:IAntBusNotification { get }
}

//MARK:AntBusChannel
fileprivate class AntBusChannel:IAntBusChannel {
    var data:IAntBusData = AntBusData()
    var router:IAntBusRouter = AntBusRouter()
    var service:IAntBusService = AntBusService()
    var notification:IAntBusNotification = AntBusNotification()
}

//MARK:AntBus extension
public extension AntBus where T == Any {
    static let channel:IAntBusChannel = AntBusChannel.init()
}
